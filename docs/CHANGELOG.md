...
### 変更（葡萄系ビッグバンド 第21弾 Albarino / 第22弾 Barbera のアーティスト画像差し替え）

- `assets/artists/albarino-budoukei-21.jpg` / `assets/artists/barbera-budoukei-22.jpg`
  - 旧：ワインボトル＋ロゴのみのビジュアル
  - 新：メンバー集合写真＋ワイン＋ロゴが入った公式ビジュアルに差し替え
  - ファイル名は据え置きのため、アーティスト個別ページ・一覧の参照はそのまま機能
...
- **Google Maps ディープリンク**：`https://www.google.com/maps/search/?api=1&query=<lat>,<lng>` を `gmaps_url(lat, lng)` ヘルパで生成。住所文字列ではなく緯度経度を直接渡すため、メインページの Leaflet マップとピン位置が完全一致。`target="_blank"` で別タブ／端末標準マップアプリが開くので、アーティストページの動線を切らずにナビできる
